...
public class ChunkHelper {
	...
	public static ChunkStub getChunkStub(ChunkCoordIntPair coord, File worldDir, World world, long worldHash) {		
		
		Chunk chunk = getChunk(coord, worldDir, world);		
		if(chunk!=null) {
	...
	protected static Chunk getChunk(ChunkCoordIntPair coord, File worldDir, World world) {
		
		Chunk chunk = null; // Utils.getChunkIfAvailable(world, coord.chunkXPos, coord.chunkZPos);
		if(chunk==null) {
			try {
				DataInputStream dis = RegionFileCache.getChunkInputStream(worldDir, coord.chunkXPos, coord.chunkZPos);
				if(dis!=null) {
					NBTTagCompound chunkNBT = CompressedStreamTools.read(dis);
					if(chunkNBT!=null) {
						chunk = checkedReadChunkFromNBT(world, coord, chunkNBT);
						if(chunk!=null) {
     ...
     */
    protected static Chunk checkedReadChunkFromNBT(World world, ChunkCoordIntPair coord, NBTTagCompound par4NBTTagCompound)
    {
    	final int par2 = coord.chunkXPos;
    	final int par3 = coord.chunkZPos;
