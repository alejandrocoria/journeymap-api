...
/**
 * JourneyMap client
 * Copyright (C) 2011-2013 Mark Woodman / TechBrew.net
 * @author mwoodman
 */
var JourneyMap = (function() {
  ...
  var initMessages = function() {
    ...
    if (JM.debug) console.log(">>> " + "initMessages");
  ...
  var initGame = function() {
    ...
    if (JM.debug) console.log(">>> " + "initGame");
    ...
    $.ajax({
        ...
        function(data, textStatus, jqXHR) {
          ...
          if (JM.game.latest_journeymap_version > JM.game.jm_version) {
            ...
            $("#versionButton").attr("title",
                JM.messages.update_available + " : JourneyMap " + JM.game.latest_journeymap_version + " for Minecraft " + JM.world.latest_minecraft_version);
  ...
  var initUI = function() {
    ...
    if (JM.debug) console.log(">>> " + "initUI");
  ...
  var initWorld = function() {
    ...
    if (JM.debug) console.log(">>> " + "initWorld");
  ...
  var initImages = function() {
    ...
    if (JM.debug) console.log(">>> " + "initImages");
  ...
  var sizeMap = function() {
    ...
    if (JM.debug) console.log(">>> " + "sizeMap");
  ...
  function setMapType(mapType) {
    ...
    if (mapType === "day") {
      ...
      if (JM.debug) console.log(">>> " + "Error: Can't set mapType: " + mapType);
    }
  ...
  var fetchData = function(dataUrl, callback) {
    ...
    if (JM.debug) console.log(">>> " + "fetchData " + dataUrl);
  ...
  var refreshMapImage = function(callback) {
    ...
    if (JM.debug) console.log(">>> " + "refreshMapImage");
    ...
    $(tempMapImage).bind('load', function() {
      ...
      if (!this.complete || typeof this.naturalWidth == "undefined" || this.naturalWidth === 0) {
        if (JM.debug) console.log(">>> " + 'Map image incomplete!');
      } else {
        latestMapImage = this;
      }
      if (callback) {
  ...
  var handleError = function(data, error, jqXHR) {
    ...
    if (JM.debug) console.log(">>> " + "handleError");
    ...
    if (!halted) {
      halted = true;
      if (JM.debug) console.log(">>> " + "Will re-check game state in 5 seconds.");
      setTimeout(function() {
  ...
  var drawMap = function() {
    ...
    if (JM.debug) console.log(">>> " + "drawMap");
  
    if (userPanning === true) {
      if (JM.debug) console.log(">>> " + "Can't draw while userPanning");
      return false;
    }
  
    if (drawingMap === true) {
      if (JM.debug) console.log(">>> " + "Avoided concurrent drawMap()");
      return false;
    }
  ...
  var drawPlayer = function(canvasWidth, canvasHeight) {
    ...
    if (JM.debug) console.log(">>> " + "drawPlayer");
  
    if (userPanning === true) return;
  ...
  var drawMobs = function(canvasWidth, canvasHeight) {
    ...
    if (JM.debug) console.log(">>> " + "drawMobs");
  ...
  var drawMultiplayers = function(canvasWidth, canvasHeight) {
    ...
    if (JM.debug) console.log(">>> " + "drawMultiplayers");
  ...
  var drawBackgroundCanvas = function(canvasWidth, canvasHeight) {
    ...
    if (JM.debug) console.log(">>> " + "drawBackgroundCanvas");
  ...
  var scrollingCanvas = function(e) {
    ...
    if (userPanning === true) {
      ...
      if (JM.debug) console.log(">>> " + "scrollingCanvas done");
