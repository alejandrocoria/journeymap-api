...
(function($) {
	...
	return this.attr('unselectable', 'on').css('user-select', 'none').on(
		'selectstart', false); // ie
    };
})(jQuery);
...
var initUI = function() {
    ...
    $("#checkSmoothScale").prop('checked', smoothScale).click(function() {
    ...
    var autoScale = ctx.imageSmoothingEnabled || ctx.mozImageSmoothingEnabled
    ...
    $("*").live('selectstart dragstart', function(evt) {
	evt.preventDefault();
	return false;
    });
	...
	dataType : "jsonp"
		...
		if (JM.game.latest_journeymap_version > JM.game.jm_version) {
		    ...
		    $("#versionButton").attr(
			    "title",
			    JM.messages.update_available + " : JourneyMap "
		...
		if (versionChecked != true) {
		    ...
		    _gaq.push([ '_trackEvent', 'Client', 'CheckVersion',
			    JM.game.jm_version ]);
		    versionChecked = true;
		}
...
var initImages = function() {
	...
	playerImage = $(document.createElement('img'))
		.attr("id", "playerImage").attr("src", "/img/locator-blue.png")
		.css("position", "absolute").css("height", "64px").css("width",
			"64px").css("cursor", "pointer").click(function() {
...
/**
 * Add a named timeout timer to the timers array, minimum being 500. If there is
 * a previous timeout already in place, the later one replaces it.
 */
...
var refreshPlayersData = function(callback) {
    ...
    if (JM.world && JM.world.singlePlayer === true)
	return;

    fetchData("/data/players", function(data) {
	JM.players = data.players;
	addTimer("refreshPlayersData", refreshPlayersData,
		JM.game.browser_playersdata_poll);
	if (callback)
	    callback();
    });
...
var refreshMobsData = function(callback) {
	...
	JM.mobs = data.mobs;
	addTimer("refreshMobsData", refreshMobsData,
		JM.game.browser_mobsdata_poll);
	if (callback)
	    callback();
    });
}
...
var refreshAnimalsData = function(callback) {
    ...
    if (showAnimals !== true && showPets !== true)
	return;

    fetchData("/data/animals?pets=" + showPets, function(data) {
	JM.animals = data.animals;
	addTimer("refreshAnimalsData", refreshAnimalsData,
		JM.game.browser_animalsdata_poll);
	if (callback)
	    callback();
    });
}
...
var refreshVillagersData = function(callback) {
    ...
    if (showVillagers !== true)
	return;

    fetchData("/data/villagers", function(data) {
	JM.villagers = data.villagers;
	addTimer("refreshVillagersData", refreshVillagersData,
		JM.game.browser_villagersdata_poll);
	if (callback)
	    callback();
    });
}
...
var refreshWorldData = function(callback) {
	...
	// 0 is the start of daytime, 12000 is the start of sunset, 13800 is the
	...
	if (callback) {
	    ...
	    addTimer("refreshWorldData", refreshWorldData,
		    JM.game.browser_mapimg_poll);
	}
...
var getMapDataUrl = function() {
    ...
    var request = "/map.png?mapType=" + mapType + "&dimension=" + dimension
	    + "&depth=" + depth + "&x1=" + mapBounds.x1 + "&z1=" + mapBounds.z1
	    + "&x2=" + mapBounds.x2 + "&z2=" + mapBounds.z2 + "&width=" + width
	    + "&height=" + height;
    return request;
}
...
var refreshMapImage = function(callback) {
    ...
    if (!tempMapImage) {
	tempMapImage = $(document.createElement('img')).error(handleError); // TODO:
									    // check
									    // whether
									    // error
									    // will
...
var refreshPlayerData = function(callback) {
	...
	dataType : "jsonp"
		...
		$("#playerLocation")
			.html(JM.player.posX + "," + JM.player.posZ);

		$("#playerElevationTitle").attr(
			'title',
			JM.messages.elevation_title + " "
				+ (JM.player.posY >> 4));
		...
		if (callback) {
		    ...
		    addTimer("refreshPlayerData", refreshPlayerData,
...
var drawMap = function() {
    ...
    if ($('body').css('backgroundColor') !== mapBackground) {
	$('body').css('backgroundColor', mapBackground);
    }
...
var drawPlayer = function(canvasWidth, canvasHeight, xOffset, zOffset) {
    ...
    if (drawingMap === true || userPanning === true)
	return;
	...
	var loc = JM.player.posX + "," + JM.player.posZ + "  ("
		+ JM.player.posY + ")"

	// Update player image
	var cursor = (centerOnPlayer === true) ? "default" : "pointer";
	var rotate = "rotate(" + player.heading + "deg)";
	$("#playerImage").attr('title', loc).css("left", x - 32).css("top",
		z - 32).css("zIndex", 2).css("cursor", cursor).css(
		"-webkit-transform", rotate).css("-moz-transform", rotate).css(
		"-o-transform", rotate).css("-ms-transform", rotate).css(
...
var drawEntity = function(ctx, mob, canvasWidth, canvasHeight, friendly) {
	...
	if (mobImage.height > 0) {
	    ctx.drawImage(mobImage, x - radius, z - radius, radius * 2,
		    radius * 2);
	}
    }
}
...
var drawBackgroundCanvas = function(canvasWidth, canvasHeight) {
	...
	var autoScale = ctx.imageSmoothingEnabled
		|| ctx.mozImageSmoothingEnabled
		|| ctx.webkitImageSmoothingEnabled;
	if (autoScale) {
	    $(ctx).prop('imageSmoothingEnabled', smoothScale).prop(
		    'mozImageSmoothingEnabled', smoothScale).prop(
		    'webkitImageSmoothingEnabled', smoothScale)
	}

	ctx.fillStyle = mapBackground;
	ctx.fillRect(0, 0, canvasWidth, canvasHeight);
	ctx
		.drawImage(latestMapImage, 0, 0, width * mapScale, height
			* mapScale);
