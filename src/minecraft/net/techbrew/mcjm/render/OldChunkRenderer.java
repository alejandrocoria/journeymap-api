...
public class OldChunkRenderer implements IChunkRenderer {
	...
	public BufferedImage getChunkImage(ChunkStub chunkStub,
		...
		try {
			...
			renderloop: if (underground) {
				...
				for (int x = 0; x < 16; x++) {
					for (int z = 0; z < 16; z++) {
						int y = chunkStub.getSafeHeightValue(x, z)-1;
	...
	public float getSlopeDiff(ChunkStub chunkStub, int x, int y, int z, Map<Integer,ChunkStub> neighbors) {
		Integer n = y, w = y, nw = y;

		if (z > 0) {
			//n = chunkStub.heightMap[(z - 1) << 4 | (x)] - 1; // north
			n = chunkStub.getSafeHeightValue(x, z-1) -1;
		} else {
			...
			if(nchunk!=null && nchunk.doMap) {
				...
				n = nchunk.getSafeHeightValue(x, 15)-1;
			} else {
				return 0; 
			}
		}
		if (x > 0) {
			...
			w = chunkStub.getSafeHeightValue(x-1, z)-1;
		} else {
			...
			if(wchunk!=null && wchunk.doMap) {
				...
				w = wchunk.getSafeHeightValue(15, z)-1;
			} else {
				return 0; 
			}
		}
		if(x>0 && z>0) {
			...
			nw = chunkStub.getSafeHeightValue(x-1, z-1)-1;
		} else if (x == 0 && z == 0) {
			...
			if(nwchunk!=null && nwchunk.doMap) {
				...
				nw = nwchunk.getSafeHeightValue(15, 15)-1;
			} else {
				return 0; 
			}
		}
	...
	public float getSlopeInSlice(ChunkStub chunkStub, final int x, final int z,
