...
public class BlockMD
{
    ...
    {
        this(block, meta, GameData.getBlockRegistry().getNameForObject(block).toString(), BlockMD.getBlockName(block, meta), 1F, 1, EnumSet.noneOf(BlockMD.Flag.class));
    }
    ...
    private static BlockMD get(Block block, int meta, Integer subBlocks)
    {
        try
        {
            ...
            if (blockMD == null)
            {
                if (GameData.getBlockRegistry().getNameForObject(block) == null)
                {
