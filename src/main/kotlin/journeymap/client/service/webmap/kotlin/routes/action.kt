...
internal fun actionGet(handler: RouteHandler): Any
{
    ...
    val world: World? = minecraft.world
...
internal fun saveMap(handler: RouteHandler, minecraft: Minecraft, world: World): Any
{
    ...
    Journeymap.getClient().toggleTask(SaveMapTask.Manager::class.java, true, mapSaver)
...
{
    val data = mutableMapOf<String, Any>()
    val enabled: Boolean = Journeymap.getClient().isTaskManagerEnabled(MapRegionTask.Manager::class.java)
    val scope: String = handler.request.queryParamOrDefault("scope", "stop")

    if (scope == "stop" && enabled)
    {
        Journeymap.getClient().toggleTask(MapRegionTask.Manager::class.java, false, false)
        data["message"] = "automap_complete"
    }
    else if (!enabled)
    {
        val doAll: Boolean = scope == "all"
        Journeymap.getClient().toggleTask(MapRegionTask.Manager::class.java, true, doAll)
